...
internal sealed class ChildProcessSupervisor(
    ILogger<ChildProcessSupervisor> logger) : IAsyncDisposable
{
    ...
    private Process? StartProcess(string productName, ProductVersionInfo version)
    {
        try
        {
            var process = StartInServiceSession(version);

            if (process is null)
            {
                logger.LogError("Не удалось запустить процесс {Product}", productName);
                return null;
            }
    ...
    private static Process? StartInServiceSession(ProductVersionInfo version)
    {
